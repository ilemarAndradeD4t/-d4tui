...
export const CheckboxField = (props: CheckBoxFieldProps) => {
  ...
  useEffect(() => {
    setItemsExtended((prevItems) => {
      return prevItems.map((item) => {
        return {
          ...item,
          selected: defaultItems?.includes(item.value),
        };
      });
    });
  }, [defaultItems]);
              ...
              <div className="flex justify-start items-end">
                ...
                {formState?.errors[id]?.message && (
                  <span className="text-xs font-light text-destructive">
                    * {formState.errors[id].message as any}
                  </span>
                )}
              </div>
                ...
                >
                  <Command style={{ width: "50px !important" }}>
                    ...
                    <CommandList>
                      ...
                      <CommandGroup className="[&_>_div]:flex [&_>_div]:w-full [&_>_div]:flex-row [&_>_div]:justify-between [&_>_div]:items-center [&_>_div]:gap-3">
                        {getSelectedItems.length > 0 && (
                          <CommandItem
                            onSelect={() => {
                              form.setValue(id, []);
